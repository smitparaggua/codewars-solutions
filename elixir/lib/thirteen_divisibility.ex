defmodule CodeWars.ThirteenDivisibility do
  @moduledoc """
  ...
  def thirt(n, previous \\ 0)
  def thirt(n, previous) when n == previous, do: n

  def thirt(n, _previous) do
    next =
      n
      |> Integer.digits()
      |> Enum.reverse()
      |> Stream.with_index()
      |> Enum.map(fn {v, i} -> v * Enum.at(@divisors, rem(i, 6)) end)
      |> Enum.sum()
